...
class PDFService {
  ...
  async generateInvoicePDF(invoiceData: ITransactionResume, forPrint: boolean = false): Promise<string> {
    const doc = new jsPDF();
    ...
    try {
      const logoWidth = 56; // Ajustar tamaño
      const logoHeight = 28; // Ajustar tamaño
      
      // Cargar la imagen
      const img = await this.loadImage(logoImage);
      
      // Crear un canvas temporal para manejar la transparencia
      ...
      if (ctx) {
        ...
        // Agregar la imagen al PDF en la esquina superior izquierda
        doc.addImage(imgData, 'PNG', margin, margin, logoWidth, logoHeight);
        
        currentY = margin + logoHeight + 5; // Dejar espacio después del logo
      } else {
    ...
    // Información de la empresa (izquierda, debajo del logo)
    doc.setFontSize(10);
    doc.setFont("helvetica", "bold");
    doc.text(invoiceData.taxpayerName ?? '', margin, currentY);
    currentY += 4.5;
    doc.setFont("helvetica", "normal");
    doc.text(`RNC ${(invoiceData as any).taxpayerId ?? ''}`, margin, currentY);
    currentY += 4.5;
    doc.text(`Dirección: C/FRANCISCO PRATTS RAMIREZ, NO. 412`, margin, currentY);
    currentY += 4.5;
    doc.text(`Telefono: (809) 565-7756`, margin, currentY);
    currentY += 4.5;
    doc.text(`Fecha Emisión: ${invoiceData.transDate}`, margin, currentY);

    // Información de la factura y cliente (derecha, arriba)
    const rightColX = pageWidth - margin; // Ajustar posición para alinear al margen derecho
    let rightColY = margin + 5;
    
    doc.setFontSize(15);
    doc.setFont("helvetica", "bold");
    rightColY += 12; // Ajuste vertical inicial
    doc.text(getCfTypeText(invoiceData.cfType), rightColX, rightColY, { align: 'right' });
    rightColY += 7;
    
    doc.setFontSize(11);
    doc.setFont("helvetica", "normal");
    doc.text(`e-NCF: ${invoiceData.cfNumber}`, rightColX, rightColY, { align: 'right' });
    rightColY += 5;
    if(invoiceData.cfType !== '32'){
      doc.text(`Fecha Vencimiento eNCF: ${invoiceData.cfValidity}`, rightColX, rightColY, { align: 'right' }); // Placeholder, ajustar si tienes este dato
    }

    // Determinar la posición inicial para la información del cliente y la tabla
    currentY = Math.max(currentY, rightColY) + 10;

    // Línea separadora después del encabezado

    
    // Información del cliente (izquierda, debajo de la línea)
    if (invoiceData.taxpayerId) {
      doc.setFontSize(11);
      doc.setFont("helvetica", "bold");
      doc.text("Cliente / Empresa: ", margin, currentY);
      currentY += 3;
      doc.line(margin, currentY, pageWidth - margin, currentY);
      currentY += 6; // Espacio después de la línea
      doc.text("Razón Social: ", margin, currentY);
      doc.setFont("helvetica", "normal");
      doc.text(invoiceData.taxpayerName ?? '', margin + 28, currentY);
      currentY += 5; // Espacio después de la línea
      doc.setFont("helvetica", "bold");
      doc.text("RNC: ", margin, currentY);
      doc.setFont("helvetica", "normal");
      doc.text(invoiceData.taxpayerId, margin + 28, currentY);
      currentY += 3;
      doc.line(margin, currentY, pageWidth - margin, currentY);
    }
    ...
    // Tabla de productos
    const tableHeaderColor = forPrint ? [255, 255, 255] : [220, 220, 220] as [number, number, number]; 
    const tableConfig = {
      startY: currentY,
      head: [['Cantidad', 'Descripción', 'Precio', 'ITBIS', 'Valor']],
      ...
      didDrawPage: function(data: any) {
        // No actualizar currentY aquí si queremos alinear el QR con los totales
      }
    };

    // Preparar datos de la tabla (incluyendo placeholders si no tienes los datos)
    const tableData = invoiceData.prods.map(item => {
      // Calcular ITBIS por item asumiendo que item.total ya incluye el impuesto
      const itemItbis = item.total - (item.quantity * item.price);
      return [
        item.quantity.toString(),
        item.productName,
        `$${formatCurrency(Number(item.price))}`,
        `$${formatCurrency(Number(itemItbis))}`,
    ...
    autoTable(doc, {
      ...tableConfig,
      body: tableData,
      // Asegurarse de que fillColor sea del tipo correcto ([number, number, number])
      headStyles: {
        ...tableConfig.headStyles,
        fillColor: tableConfig.headStyles.fillColor as [number, number, number]
      },
      styles: {
        ...tableConfig.styles,
        lineColor: tableConfig.styles.lineColor as [number, number, number]
      }
    });

    // Determinar la posición Y después de la tabla para los totales y el QR
    const yAfterTable = (doc as any).lastAutoTable?.finalY || (currentY + (tableData.length * 8) + 10);
    let totalsY = yAfterTable + 10;

    // Totales (derecha, debajo de la tabla)
    ...
    // Subtotal Gravado
    doc.text('Subtotal Gravado:', totalsValueX - totalLabelOffset, totalsY, { align: 'right' });
    doc.rect(totalsValueX - 40, totalsY - 5, 40, totalRowHeight);
    doc.text(`$${formatCurrency(Number(invoiceData.subtotal))}`, totalsValueX - 2, totalsY, { align: 'right' });
    totalsY += totalRowHeight;

    // Total ITBIS
    doc.text('Total ITBIS:', totalsValueX - totalLabelOffset, totalsY, { align: 'right' });
    doc.rect(totalsValueX - 40, totalsY - 5, 40, totalRowHeight);
    doc.text(`$${formatCurrency(Number(invoiceData.tax))}`, totalsValueX - 2, totalsY, { align: 'right' });
    totalsY += totalRowHeight;

    // Total General
    doc.text('Total:', totalsValueX - totalLabelOffset, totalsY, { align: 'right' });
    doc.rect(totalsValueX - 40, totalsY - 5, 40, totalRowHeight);
    doc.text(`$${formatCurrency(Number(invoiceData.total))}`, totalsValueX - 2, totalsY, { align: 'right' });
    ...
    try {
      ...
      const qrDataUrl = await QRCode.toDataURL(invoiceData.cfQr ?? '');
      doc.addImage(qrDataUrl, 'PNG', qrX, qrY, qrSize, qrSize);
      
      // Agregar etiquetas debajo del QR (alineadas con el margen)
      doc.setFontSize(11);
      doc.setFont("helvetica", "normal");
      doc.text(`Código de Seguridad: ${invoiceData.cfSecurityCode}`, margin, qrY + qrSize + 3   );
      //separar con un espacio de 2

      doc.text(`Fecha de Firma Digital: ${invoiceData.digitalSignatureDate}`, margin, qrY + qrSize + 6 + 3);
    } catch (error) {
    ...
    if (forPrint) {
      ...
      if(invoiceData.taxpayerId){
        doc.save(`factura-${invoiceData.cfNumber}-${invoiceData.taxpayerId}.pdf`);
      }else{
        doc.save(`factura-${invoiceData.cfNumber}.pdf`);
      }
      return '';
    }
  }
