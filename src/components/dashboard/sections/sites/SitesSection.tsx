...
import { Building2, Search, Filter, RefreshCw, Download, Plus, Edit, Trash2, Eye, CheckCircle, XCircle, Phone, Mail, MapPin, DollarSign } from 'lucide-react';
...
const SitesSection: React.FC = () => {
  const { sites, loading, error, refreshSites, createSite, updateSite, deleteSite } = useSites();
    ...
    <div className="space-y-6">
                ...
                <div className="flex items-center space-x-4">
                  <div className="bg-white px-4 py-2 rounded-lg border border-gray-200 min-w-[120px]">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-lg font-bold text-gray-900">{totalSites}</span>
                  ...
                  <div className="bg-white px-4 py-2 rounded-lg border border-gray-200 min-w-[120px]">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-lg font-bold text-green-600">{activeSites}</span>
                  ...
                  <div className="bg-white px-4 py-2 rounded-lg border border-gray-200 min-w-[120px]">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-lg font-bold text-red-600">{inactiveSites}</span>
                  ...
                  <div className="bg-white px-4 py-2 rounded-lg border border-gray-200 min-w-[120px]">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-lg font-bold text-blue-600">{posSites}</span>
              ...
              {currentSites.map((site) => (
                   ...
                   <td className="px-6 py-4">
                     <div className="flex items-center">
                       ...
                       <div className="ml-4">
                         <div className="text-sm font-medium text-gray-900">{site.site_id} {site.name}</div>
                         <div className="text-xs text-gray-400">Número: {site.site_number}</div>
                       </div>
                     </div>
                   </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      <span className={`ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(site.active)}`}>
      ...
      <div className="flex items-center justify-between bg-white px-6 py-3 border border-gray-200 rounded-lg">
        <div className="text-sm text-gray-700">
          ...
          <span className="font-medium">{Math.min(endIndex, filteredSites.length)}</span> de{' '}
          <span className="font-medium">{filteredSites.length}</span> sucursales
          {filteredSites.length !== sites.length && (
            <span className="text-gray-500"> (filtradas de {sites.length} total)</span>
          )}
        </div>
        <div className="flex items-center space-x-2">
          <button 
